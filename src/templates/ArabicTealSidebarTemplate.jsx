...
const ArabicTealSidebarTemplate = ({
  ...
  const sb = {
    ...
    jobTitle:{ fontSize:sz.meta, color:'rgba(255,255,255,0.7)', textAlign:'center', marginBottom:'14pt', fontStyle:'italic' },
    sectionTitle:{
      fontSize:'9pt', fontWeight:'700', color:'#fff',
      textAlign:'right', letterSpacing:'0.05em',
      borderBottom:'1px solid rgba(255,255,255,0.25)',
      paddingBottom:'4pt', marginBottom:'8pt', marginTop: sectionMt,
      ...BREAK_HEADING,
    },
    contactRow:{
  ...
  const mn = {
    ...
    jobBadge:{
      ...
      color:'#fff', borderRadius:'3pt', padding:'2pt 10pt',
      fontSize:sz.meta, fontWeight:'600', marginBottom:'12pt',
    },
    summaryText:{ fontSize:sz.body, color:'#555', lineHeight, textAlign:'right', whiteSpace:'pre-line', marginBottom:sectionMt },
    heading:{
      fontSize:sz.heading, fontWeight:'700', color: accentDark,
    ...
    company:{ fontSize:sz.meta, color: accentDark, fontWeight:'600', marginBottom:'3pt', textAlign:'right' },
    bodyText:{ fontSize:sz.body, color:'#555', lineHeight, whiteSpace:'pre-line', textAlign:'right' },
    bullet:{ fontSize:sz.body, color:'#444', marginBottom:'3pt', paddingRight:'8pt', textAlign:'right' },
        ...
        <section key="languages" style={BREAK_ITEM}>
          ...
          {data.languages.map((l,i)=>(
            <div key={i} style={sb.langRow}>
              <Dots level={l.proficiency||3} filled="#fff" empty="rgba(255,255,255,0.25)" />
      ...
      case 'courses': {
        ...
        return (
          <div key={key} style={BREAK_ITEM}>
            <div style={sb.sectionTitle}>{tr(key,true)}</div>
            {items.map((c,i)=>(
              <div key={i} style={{ marginBottom:'7pt' }}>
                ...
                {c.date && <div style={{ ...sb.bullet, fontSize:'8pt', color:'rgba(255,255,255,0.55)' }}>{c.date}</div>}
              </div>
            ))}
          </div>
        );
        ...
        <section key="experience">
          {data.experience.map((e,i)=>(
            <div key={i} style={mn.item}>
              {i === 0 && <div style={mn.heading}>{tr('experience',true)}</div>}
              <div style={mn.row}>
      ...
      case 'education': return data.education?.length > 0 ? (
        <section key="education">
          {data.education.map((e,i)=>(
            <div key={i} style={mn.item}>
              {i === 0 && <div style={mn.heading}>{tr('education',true)}</div>}
              <div style={mn.row}>
      ...
      case 'projects': return data.projects?.length > 0 ? (
        <section key="projects">
          {data.projects.map((p,i)=>(
            <div key={i} style={mn.item}>
              {i === 0 && <div style={mn.heading}>{tr('projects',true)}</div>}
      ...
      case 'publications': return data.publications?.length > 0 ? (
        <section key="publications">
          {data.publications.map((p,i)=>(
            <div key={i} style={mn.item}>
              {i === 0 && <div style={mn.heading}>{tr('publications',true)}</div>}
              <div style={mn.row}>
      ...
      case 'references': return data.references?.length > 0 ? (
        <section key="references">
          {data.references.map((r,i)=>(
            <div key={i} style={mn.item}>
              {i === 0 && <div style={mn.heading}>{tr('references',true)}</div>}
  ...
  return (
    <div style={{ fontFamily:font, fontSize:sz.body, color:'#1a202c', backgroundColor:'#fff', width:'794px', minHeight:'1122px', boxSizing:'border-box', display:'flex', flexDirection:'row', direction:'rtl' }}>
      ...
      <div style={sb.wrap}>
        ...
        {/* Contact */}
        <div style={sb.sectionTitle}>{tr('contact',true)}</div>
      ...
      <div style={mn.wrap}>
        {/* Name block */}
        <div style={{ textAlign:'right', marginBottom:'10pt' }}>
          <div style={mn.name}>{info.fullName||'الاسم الكامل'}</div>
          {info.jobTitle && <span style={mn.jobBadge}>{info.jobTitle}</span>}
        </div>
...
export default ArabicTealSidebarTemplate;
