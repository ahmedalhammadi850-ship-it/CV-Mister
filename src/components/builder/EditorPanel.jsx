              ...
              <div className="p-4 space-y-3 bg-slate-50/50 border-b border-slate-100">
                {(cvData.projects || []).map((proj, i) => (
                  <CardWrapper key={proj.id} onDelete={() => updateSection('projects', cvData.projects.filter(p => p.id !== proj.id))}>
                    ...
                    <div><label className={lbl}>{t('Description','الوصف',isRTL)}</label><AITextarea className={`${inp} resize-none`} rows={3} value={proj.description} onChange={val => updateSection('projects', cvData.projects.map(p => p.id === proj.id ? { ...p, description: val } : p))} isRTL={isRTL} placeholder={isRTL ? 'صف المشروع...' : 'Describe the project...'} align={proj.descriptionAlign} onAlignChange={val => updateSection('projects', cvData.projects.map(p => p.id === proj.id ? { ...p, descriptionAlign: val } : p))} bold={proj.descriptionBold ?? false} onBoldChange={val => updateSection('projects', cvData.projects.map(p => p.id === proj.id ? { ...p, descriptionBold: val } : p))} italic={proj.descriptionItalic ?? false} onItalicChange={val => updateSection('projects', cvData.projects.map(p => p.id === proj.id ? { ...p, descriptionItalic: val } : p))} /></div>
                  </CardWrapper>
                ))}
              ...
              <div className="p-4 space-y-3 bg-slate-50/50 border-b border-slate-100">
                {(cvData.awards || []).map((award) => (
                  <CardWrapper key={award.id} onDelete={() => updateSection('awards', cvData.awards.filter(a => a.id !== award.id))}>
                    ...
                    <div><label className={lbl}>{t('Description','الوصف',isRTL)}</label><AITextarea className={`${inp} resize-none`} rows={2} value={award.description || ''} onChange={val => updateSection('awards', cvData.awards.map(a => a.id === award.id ? { ...a, description: val } : a))} isRTL={isRTL} placeholder={isRTL ? 'صف الجائزة...' : 'Describe the award...'} align={award.descriptionAlign} onAlignChange={val => updateSection('awards', cvData.awards.map(a => a.id === award.id ? { ...a, descriptionAlign: val } : a))} bold={award.descriptionBold ?? false} onBoldChange={val => updateSection('awards', cvData.awards.map(a => a.id === award.id ? { ...a, descriptionBold: val } : a))} italic={award.descriptionItalic ?? false} onItalicChange={val => updateSection('awards', cvData.awards.map(a => a.id === award.id ? { ...a, descriptionItalic: val } : a))} /></div>
                  </CardWrapper>
                ))}
              ...
              <div className="p-4 space-y-3 bg-slate-50/50 border-b border-slate-100">
                {(cvData.publications || []).map((pub) => (
                  <CardWrapper key={pub.id} onDelete={() => updateSection('publications', cvData.publications.filter(p => p.id !== pub.id))}>
                    ...
                    <div><label className={lbl}>{t('Description','الوصف',isRTL)}</label><AITextarea className={`${inp} resize-none`} rows={2} value={pub.description || ''} onChange={val => updateSection('publications', cvData.publications.map(p => p.id === pub.id ? { ...p, description: val } : p))} isRTL={isRTL} placeholder={isRTL ? 'صف المنشور...' : 'Describe the publication...'} align={pub.descriptionAlign} onAlignChange={val => updateSection('publications', cvData.publications.map(p => p.id === pub.id ? { ...p, descriptionAlign: val } : p))} bold={pub.descriptionBold ?? false} onBoldChange={val => updateSection('publications', cvData.publications.map(p => p.id === pub.id ? { ...p, descriptionBold: val } : p))} italic={pub.descriptionItalic ?? false} onItalicChange={val => updateSection('publications', cvData.publications.map(p => p.id === pub.id ? { ...p, descriptionItalic: val } : p))} /></div>
                  </CardWrapper>
                ))}
              ...
              <div className="p-4 space-y-3 bg-slate-50/50 border-b border-slate-100">
                ...
                {sec.items.map(item => (
                  <CardWrapper key={item.id} onDelete={() => updateCustomSection(sec.id, { ...sec, items: sec.items.filter(i => i.id !== item.id) })}>
                    ...
                    <div><label className={lbl}>{t('Description', 'الوصف', isRTL)}</label><AITextarea className={`${inp} resize-none`} rows={3} value={item.description || ''} onChange={val => updateCustomSection(sec.id, { ...sec, items: sec.items.map(i => i.id === item.id ? { ...i, description: val } : i) })} isRTL={isRTL} placeholder={isRTL ? 'صف العنصر...' : 'Describe this item...'} align={item.descriptionAlign} onAlignChange={val => updateCustomSection(sec.id, { ...sec, items: sec.items.map(i => i.id === item.id ? { ...i, descriptionAlign: val } : i) })} bold={item.descriptionBold ?? false} onBoldChange={val => updateCustomSection(sec.id, { ...sec, items: sec.items.map(i => i.id === item.id ? { ...i, descriptionBold: val } : i) })} italic={item.descriptionItalic ?? false} onItalicChange={val => updateCustomSection(sec.id, { ...sec, items: sec.items.map(i => i.id === item.id ? { ...i, descriptionItalic: val } : i) })} /></div>
                  </CardWrapper>
                ))}
                <AddBtn
