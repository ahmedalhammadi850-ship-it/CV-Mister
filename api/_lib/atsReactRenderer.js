...
function _buildDocument(bodyHtml, isRTL, pageBreaks, totalHeight) {
  ...
  const baseStyles = `
    ...
    *, *::before, *::after {
    ...
    .cv-heading {
      /* Keep heading with its content (no orphan) AND prevent heading from being split */
      break-after: avoid;
    ...
    /* Prevent heading elements (h1-h6) from being split horizontally.
     * Templates render job titles as <h3>, section labels as <h2>, etc.
     * Without this rule a large item can be split mid-heading. */
    h1, h2, h3, h4, h5, h6 {
      break-inside: avoid;
      page-break-inside: avoid;
    }
